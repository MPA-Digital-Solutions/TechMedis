...
export function SubcategoryFilter({ category }: SubcategoryFilterProps) {
  ...
  const handleFilterChange = (slug: string | null) => {
    if (slug) {
      ...
      router.push(`${basePath}/${category}`, { scroll: false });
    }
  };
    ...
    <div className="flex flex-wrap gap-3 mb-8">
      ...
      {subcategories.map((sub) => (
        <button
          ...
          className={`px-4 py-2 rounded-lg font-medium transition-all duration-200 cursor-pointer ${currentSubcategory === sub.slug
              ? "bg-techmedis-primary text-white shadow-lg"
